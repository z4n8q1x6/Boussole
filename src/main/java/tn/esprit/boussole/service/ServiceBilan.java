...
public class ServiceBilan implements crud<bilan> {
    ...
    @Override
    public List<bilan> selectAll(bilan bilan) throws SQLException {
        return List.of();
    }
    ...
    public void genererBilan(int mois, int annee, int franchiseId) {
        if (franchiseId == 0) {
            // TOUT LE RÉSEAU : générer un bilan pour CHAQUE franchise
            String sqlFranchises = "SELECT DISTINCT franchise_id FROM transaction WHERE franchise_id IS NOT NULL AND franchise_id != 0";
            try (PreparedStatement ps = cnx.prepareStatement(sqlFranchises);
                 ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    int fid = rs.getInt("franchise_id");
                    genererBilanPourFranchise(mois, annee, fid);
                }
            } catch (SQLException e) {
                System.err.println("Erreur listing franchises: " + e.getMessage());
            }
        } else {
            genererBilanPourFranchise(mois, annee, franchiseId);
        }
    }

    private void genererBilanPourFranchise(int mois, int annee, int franchiseId) {
        String sqlRecettes = "SELECT COALESCE(SUM(montant), 0.0) AS total FROM transaction WHERE MONTH(date) = ? AND YEAR(date) = ? AND type = 'RECETTE' AND franchise_id = ?";
        String sqlDepenses = "SELECT COALESCE(SUM(montant), 0.0) AS total FROM transaction WHERE MONTH(date) = ? AND YEAR(date) = ? AND type = 'DEPENSE' AND franchise_id = ?";
        ...
        try (PreparedStatement psRec = cnx.prepareStatement(sqlRecettes);
            ...
            psRec.setInt(3, franchiseId);
            try (ResultSet rs = psRec.executeQuery()) {
                if (rs.next()) totalRecettes = rs.getDouble("total");
            }

            psDep.setInt(1, mois);
            psDep.setInt(2, annee);
            psDep.setInt(3, franchiseId);
            try (ResultSet rs = psDep.executeQuery()) {
                if (rs.next()) totalDepenses = rs.getDouble("total");
            }

            double resultatNet = totalRecettes - totalDepenses;

            // Vérifier si un bilan existe déjà pour ce mois/année/franchise
            String sqlCheck = "SELECT id FROM bilan WHERE mois = ? AND annee = ? AND franchise_id = ?";
            try (PreparedStatement psCheck = cnx.prepareStatement(sqlCheck)) {
                psCheck.setInt(1, mois);
                psCheck.setInt(2, annee);
                psCheck.setInt(3, franchiseId);
                try (ResultSet rsCheck = psCheck.executeQuery()) {
                    if (rsCheck.next()) {
                        // UPDATE existant
                        int existingId = rsCheck.getInt("id");
                        String sqlUpdate = "UPDATE bilan SET total_recettes = ?, total_charges = ?, resultat_net = ? WHERE id = ?";
                        try (PreparedStatement psUpdate = cnx.prepareStatement(sqlUpdate)) {
                            psUpdate.setDouble(1, totalRecettes);
                            psUpdate.setDouble(2, totalDepenses);
                            psUpdate.setDouble(3, resultatNet);
                            psUpdate.setInt(4, existingId);
                            psUpdate.executeUpdate();
                        }
                        System.out.println("Bilan MIS À JOUR : Mois=" + mois + ", Année=" + annee + ", FranchiseId=" + franchiseId + ", Recettes=" + totalRecettes + ", Dépenses=" + totalDepenses);
                    } else {
                        // INSERT nouveau
                        bilan b = new bilan();
                        b.setMois(mois);
                        ...
                        insertone(b);
                        System.out.println("Bilan CRÉÉ : Mois=" + mois + ", Année=" + annee + ", FranchiseId=" + franchiseId + ", Recettes=" + totalRecettes + ", Dépenses=" + totalDepenses);
                    }
                }
            }

        } catch (SQLException e) {
            System.err.println("Erreur génération bilan: " + e.getMessage());
        }
    }
