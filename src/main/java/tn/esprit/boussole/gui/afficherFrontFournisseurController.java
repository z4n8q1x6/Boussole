...
public class afficherFrontFournisseurController {
  ...
  private static final String OPENROUTER_API_KEY =
      "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";
  ...
  public void initialize() {
    // --- STYLE IA ---
    txtReponseIA.setStyle(
        "-fx-control-inner-background: #0C0F1A; -fx-text-fill: #00E5CC; -fx-font-family: 'Segoe"
            + " UI';");
        ...
        new Task<>() {
          ...
          protected String call() throws Exception {
            try {
              ...
              String urlStr =
                  "https://api.open-meteo.com/v1/forecast?latitude=36.8188&longitude=10.1659&current_weather=true";
  ...
  private void handleAnalyseIA() {
    ...
    String noms =
        allFournisseurs.stream().map(Fournisseur::getNom).collect(Collectors.joining(", "));
    ...
    Task<String> task =
        new Task<>() {
          ...
          protected String call() throws Exception {
            ...
            msg.put(
                "content",
                "En tant qu'expert logistique, donne 2 conseils courts et stratégiques pour gérer"
                    + " ces fournisseurs : "
                    + noms);
            messages.put(msg);
  ...
  private void loadData() {
    try {
      allFournisseurs = fournisseurService.selectAll(null);
      applyFilters();
  ...
  private void applyFilters() {
    ...
    if (sortOption != null) {
      if (sortOption.equals("Matricule Croissant"))
        ...
        displayedFournisseurs.sort(
            Comparator.comparing(Fournisseur::getMatriculeFiscal).reversed());
    }
  ...
  private VBox createFournisseurCard(Fournisseur fournisseur) {
    ...
    String defaultStyle =
        "-fx-background-color: #0C0F1A; -fx-background-radius: 15; -fx-border-color:"
            + " rgba(255,255,255,0.1); -fx-border-radius: 15;";
    String hoverStyle =
        "-fx-background-color: #1E293B; -fx-background-radius: 15; -fx-border-color: #0EA5E9;"
    ...
    lblPhone.setStyle(
        "-fx-background-color: rgba(0, 229, 204, 0.1); -fx-padding: 5 10; -fx-background-radius:"
            + " 20;");
  ...
  private void openBusinessLink() {
    try {
      if (Desktop.isDesktopSupported()) Desktop.getDesktop().browse(new URI(businessNewsUrl));
    } catch (Exception e) {
